...
OUTPUT_DIR = "results"
OUTPUT_TYPE = "png"
TIMES, N_USERS, N_SLICES = 3, 194, 20
...
def process_log(fname):
    ...
    df_cumu_tmp = grp.max()
    df_cumu = df_cumu_tmp.reset_index()
    app_to_slice = pd.Series({app: s for app, s in df_cumu_tmp.index})
    ...
    # Head-of-line delays
    hol_array = df[df["slice"].between(15, 19)]["hol_delay"].tolist()

    # Flow completion times
    fct_array = df_ipflow[app_to_slice[df_ipflow["app"]].between(5, 9).array]["fct"].tolist()
...
def plot_together(enterprise_alg):
    ...
    for i in range(0, TIMES):
        for k, v in LOG_MAPPING.items():
            slice_rbs, slice_throughput, hols, fcts = process_log(
                f"{INPUT_DIR}/{enterprise_alg}-{v}-{i}.log"
            )
    ...
    plt.tight_layout()
    location_tprbs = f"{OUTPUT_DIR}/{enterprise_alg}-tprbs.{OUTPUT_TYPE}"
    ...
    plt.tight_layout()
    location_fcthol = f"{OUTPUT_DIR}/{enterprise_alg}-fcthol.{OUTPUT_TYPE}"
...
if __name__ == "__main__":
    ...
    intra_algs = args.intra_alg
    if intra_algs is None:
        raise ValueError("--intra-alg must be specified")
    intra_algs = AVAIL_INTRA_ALGS if "all" in intra_algs else set(intra_algs)
    suites = list(intra_algs)

    if not os.path.exists(OUTPUT_DIR):
        os.makedirs(OUTPUT_DIR)

    n_suites = len(suites)
    for i, enterprise_alg in enumerate(suites):
        print()
        print(
            Fore.BLUE
            + Style.BRIGHT
            + f"[{i + 1}/{n_suites}] Enterprise: {enterprise_alg}"
        )
        loc_tprbs, loc_fcthol = plot_together(enterprise_alg)
        print()
